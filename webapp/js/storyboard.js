...
globalThis.SB = {
  ...
  saveAgain: false,
  renderMode: (function () {
    try { return localStorage.getItem('phos_sb_render_mode') || 'images'; }
...
function sbRenderPlan(r) {
  ...
  // --- render mode & master ref ---
  const curMode = SB.renderMode || 'images';
  const modeGroup = sbEl('sbRenderModeGroup');
  if (modeGroup) {
    modeGroup.querySelectorAll('.pill-btn').forEach(btn => {
      btn.classList.toggle('active', btn.dataset.sbRendermode === curMode);
    });
  }
  ...
  if (curMode === 'images') {
    btn.textContent = '📸 Generate All Image Stills';
    btn.onclick = () => sbRenderImages();
  } else {
    btn.textContent = '🎬 Render All Video Drafts';
    btn.onclick = () => sbRenderDrafts();
  }

  const convertAllBtn = sbEl('sbConvertAllBtn');
  if (convertAllBtn) {
    const hasImages = shots.some(s => s.image_output);
    convertAllBtn.style.display = (curMode === 'images' && hasImages) ? '' : 'none';
  }
    ...
    <div class="sb-shot-out car-card sb-shot-imgcard" style="margin-top:6px;border:1px solid rgba(139,123,255,0.4);background:rgba(139,123,255,0.04);border-radius:8px;padding:8px;">
      <div style="display:flex;align-items:center;gap:12px;">
        ...
        <div class="info" style="flex:1;">
          ...
          <span class="sub" style="font-size:11px;color:var(--muted);">${escapeHtml(imgPath.split('/').pop())}</span>
        </div>
        <div class="sb-grade" role="group" aria-label="Shot ${n} Image Actions" style="display:flex;gap:6px;">
          <button type="button" class="ghost-btn" style="padding:4px 8px;font-size:11px;" onclick="sbRenderImages([${n}])" title="Re-generate this image still">📸 Re-gen Image</button>
          <button type="button" class="primary" style="padding:4px 10px;font-size:11px;" onclick="sbConvertShotToVideo(${n})" title="Convert approved image still to video draft">🎞️ Convert to Video</button>
        </div>
      </div>
    </div>` : (SB.renderMode === 'images' ? `
    <div class="sb-shot-img-actions" style="margin-top:6px;display:flex;gap:8px;align-items:center;">
      <button type="button" class="ghost-btn" style="padding:4px 10px;font-size:12px;" onclick="sbRenderImages([${n}])">📸 Generate Image Still</button>
      <span class="hint" style="font-size:11px;color:var(--muted);">Generate a still image to preview shot ${n} before video</span>
    </div>` : '');
...
// ---- render mode & image-first workflow -----------------------------
...
async function sbRenderImages(only) {
  ...
  if (!r.ok) {
    phosToast(r.error || 'Could not queue image generation.', { kind: 'danger', duration: 6000 });
    return;
  }
  phosToast(`Queued image generation for ${r.queued || 1} shot(s).`, { kind: 'success' });
  sbLoad(SB.id);
  if (typeof poll === 'function') poll();
}

async function sbConvertShotToVideo(n) {
  if (!SB.id) return;
  const fd = new URLSearchParams();
  fd.set('id', SB.id);
  fd.set('n', String(n));
  let r;
  try { r = await (await fetch('/storyboard/convert_shot_to_video', { method: 'POST', body: fd })).json(); }
  catch (e) { r = { ok: false, error: String(e) }; }
  if (!r.ok) {
    phosToast(r.error || 'Could not convert image to video.', { kind: 'danger', duration: 6000 });
    return;
  }
  phosToast(`Queued Shot ${n} image-to-video job!`, { kind: 'success' });
  sbLoad(SB.id);
  if (typeof poll === 'function') poll();
}

async function sbConvertAllToVideo() {
  if (!SB.id) return;
  const fd = new URLSearchParams();
  fd.set('id', SB.id);
  let r;
  try { r = await (await fetch('/storyboard/convert_all_to_video', { method: 'POST', body: fd })).json(); }
  catch (e) { r = { ok: false, error: String(e) }; }
  if (!r.ok) {
    phosToast(r.error || 'Could not convert images to video.', { kind: 'danger', duration: 6000 });
    return;
  }
  phosToast(`Queued ${r.queued || 0} video job(s) from shot images!`, { kind: 'success' });
  sbLoad(SB.id);
  if (typeof poll === 'function') poll();
}
