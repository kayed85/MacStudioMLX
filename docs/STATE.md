...
> - **NOT done here, still open** (schema v2, `~/AI/projects/phosphene/notes/analytics_spec_2026-08-11.md`): `$geoip_disable` + `$ip: null` on the payload (GeoIP is still on by default) — ⚠️ **SUPERSEDED 2026-08-12 by the GeoIP-kill entry above; and `$ip: null` as written here is a no-op, see there for what actually works**, `error_class` replacing free-text `error_signature`, version/chip context on render events, `update_state`, `wall_sec_bucket`. Phase 0 was shipped alone deliberately — the doc had to stop being false before anything else was worth arguing about.
...
> record actually lives. The dated blockquote log at the top of this file is the engineering
> handoff, not a user changelog. v3.8.0 is written up here as well because a release that
> changes the install path deserves a line somebody can find with grep.

#### v3.8.3 — the install that hung Pinokio before it ever started
- **Symptom.** On Pinokio 8.0.40 a fresh install freezes at ~100% CPU immediately after
  `git rev-parse --short HEAD`. No child process spawns, nothing is logged, and the only way
  out is force-quitting Pinokio. Reported by **@davidaircloud** on a completely clean
  Pinokio 8.0.40 + Phosphene 3.8.0 (#50, reopened after the v3.6.2 fix) and independently by
  **@itrejomx** (#56), who saw the same stall before `uv venv` and fixed it locally by
  splitting the payload into smaller steps.
- **Cause — the rule was right and the measurement was wrong.** @Morac2 bisected the ceiling
  in #50: a 764-char command hangs, ~373 passes. v3.6.2 adopted *"no generated shell line over
  350"* and the issue was closed. But what Pinokio writes to the pty is not a line of our
  source. `pinokiod/kernel/shells.js` `launch()` dispatches an **array** message one element
  per shell, and a **string** message in ONE unchunked write (`shell.js:1911`) — and every
  long payload in this repo is `[ ... ].join("\n")`, deliberately, so an `if/else` runs in a
  single shell. Those were exactly the dispatches nobody was counting. The LTX venv step's
  source lines were all under 110 chars and it handed Pinokio **1,417** characters in one go —
  the exact number @davidaircloud measured himself, and the exact thing he fixed by moving the
  body into a `.sh`.
- **Corroboration from Pinokio's own source.** `Shell.emit()` (shell.js:665-711) chunks
  anything over 1024 chars into 256-byte pieces with a 10 ms gap, so the kernel already treats
  large pty writes as unsafe; the `shell.run` dispatch path simply does not do it. A local pty
  probe using Pinokio's own shell and args (`bash --noprofile --norc`, one unchunked write)
  completes a 1,475-byte payload fine — so this is **not** a kernel tty limit, and dispatch
  size is the only lever available from this side.
- **The fix.** Four oversized dispatches move into `scripts/pinokio/`, each step becoming one
  short `bash …` line: the LTX venv check (1,417), the mflux image-engine pack (529), the H3
  memory preflight (845) and the H3 Q8 engine build (595). Worst dispatch in the tree is now
  **498**. Every script is a verbatim lift, run as `bash <path>` so no execute bit is involved,
  keeping the `\n`-joined steps' no-`set -e` semantics. The vendored engine pin is unchanged
  and still lives in the `.js` beside the notes that document it.
- **Proof.** Each old inline payload and its new script were run in byte-identical sandboxes
  across every branch — healthy venv, broken venv, macOS 13 abort, mflux, H3 preflight at
  32 / 48 / 64 GB, Q8 already-built / build / skip — and compared on stdout and exit code.
  10/10 identical; the only difference is that bash's own diagnostic now names the script
  instead of saying `bash`.
- **The gate.** `scripts/check_pinokio_scripts.js` loads each launcher module and models
  `launch()` to find the real dispatches, failing anything over 500 chars. 500 sits above the
  largest dispatch that has demonstrably shipped and worked (498) and well under the smallest
  that has demonstrably hung (764). Six dispatches sit in the 352-498 band; they work today and
  tightening toward 350 belongs in a feature release, not a hotfix.
- **Also — the H3 install pointed at a control that had moved (#58).** After a 75 GB download,
  `install_h3.js` said *"the Video tab now has an Engine row"*. The engine control has been the
  switcher in the **top right of the header** since the engine table landed; there is no Engine
  row in the Video tab to find, and #58 is a user who installed H3 successfully and went
  looking there. The notification and `docs/H3_ENGINE.md` now say where it is, and name the one
  condition that genuinely hides it: the switcher only appears when more than one engine is
  renderable, and on a sub-60 GB Mac `h3_capable()` stays false until the reduced-RAM Q8 pack
  exists on disk (`dit_choice.q8_available` in `/status`).

#### v3.8.2 — the codec patch never ran, and nothing noticed
- **Symptom: none.** That is what made it dangerous. After updating to v3.8.1 the app reported
  a finished update, the engine pin had moved, the version handshake was green — and
  `video_vae.py` in site-packages was still upstream. Every render was encoded 4:2:0
  `yuv420p`: no error, no warning, just blocky compression on faces and skin, which is the one
  thing Phosphene patches the encoder to avoid.
- **Cause 1 — the run truncated.** Pinokio's own session log for a real v3.1.1 → v3.8.1 update
  records steps 0–7 of 18 and then nothing. The last step executed was `certifi`, a plain
  `pip install --upgrade`, whose output carried pip's *"ERROR: pip's dependency resolver does
  not currently take into account all the packages that are installed"* block — a permanent
  side effect of `mlx-vlm` being installed `--no-deps`, which makes pip's post-install
  consistency pass complain after any step that actually changes something. pip still exits 0
  (measured); the run ended anyway, and every step after it was skipped in silence. Confirmed
  on that install: no `smolagents`, no `mflux`, no `mlx-teacache` — those steps had not run in
  a long time either.
- **Cause 2 — the patch sat too late.** It was eleven steps down, behind litellm, smolagents,
  mflux and the IC-LoRA fetches: all optional, all able to end the run before the one step
  that is not optional.
- **Cause 3 — the patch script guessed.** `_find` walked a hardcoded list of relative paths
  resolved against the working directory, so running it by hand from anywhere but the app root
  reported `MISSING — target file not found` on an install whose file was sitting right there.
- **Fixes.** The codec patch now runs *immediately* after the uv reinstall that replaces
  site-packages, before any optional step exists to truncate the run — and a gate enforces that
  ordering. Every remaining plain-pip step in `update.js` is now a uv step (same installs, same
  broken metadata, zero resolver-ERROR output — measured side by side); `install.js` was
  already all-uv, which is why fresh installs patched correctly. `patch_ltx_codec.py` resolves
  its target through the running interpreter's own import machinery — the file the panel will
  actually import, editable install or not — with the path list kept only as a fallback,
  anchored on the script's directory rather than the CWD. And after patching it re-reads the
  resolved file, asserts both markers, and exits non-zero behind a banner if they are absent.
- **If you rendered anything on v3.8.1, re-render it after updating.** Those clips are 4:2:0.
  Nothing else is affected: no re-download, no weight change, no pin change (still `871694d` /
  `0.14.19+ltx25.1`).

#### v3.8.1 — the Update that could not be retried
- **Update / Install aborted at the video-engine pin, permanently.** Both paths move the
  vendored `ltx-2-mlx` checkout with `git checkout <sha>`, and for an affected install that
  line failed with `error: Your local changes to the following files would be overwritten by
  checkout: packages/ltx-core-mlx/src/ltx_core_mlx/model/video_vae/video_vae.py`. Nothing in
  the flow ever un-dirtied the file, so every retry died on the same line — including "click
  Install again", the recovery we tell people to use.
- **Where the dirty file came from — every install, not a rare cohort.** `ltx-2-mlx` is a uv
  *workspace*, and `install.js` installs its members with deps and without `--reinstall`, so uv
  links them **editable**: site-packages gets `_editable_impl_ltx_core_mlx.pth`, not a copy.
  `patch_ltx_codec.py` resolves its target through `VENV_ROOTS` — site-packages first,
  `packages/ltx-core-mlx/src` as a fallback — finds no `ltx_core_mlx` directory in
  site-packages, and patches the **git-tracked source**. A perfectly successful install
  therefore ends with a modified tracked file, every time. (An install that *died* at the
  package step lands in the same place for a different reason: nothing in site-packages at
  all.) The `v0.14.8 → 871694d` pin move rewrites the very ffmpeg lines that patch edits, so
  git refused to move the pin.
- **The fix.** Both `install.js` and `update.js` discard local edits in the vendored checkout
  (`git reset --hard HEAD`, behind a guard that the shell really landed in that tree) before
  moving the pin. The checkout is app-managed: models live in `../mlx_models`, the build
  constraints in `../pip-build-constraints.txt`, and the only non-tracked thing inside it on a
  real install is the ignored `env/` venv — which `reset --hard` does not touch. Update's own
  `uv pip install --reinstall --no-deps` step then replaces the editable `.pth` links with real
  copies in site-packages, and the codec patch re-applies **there** — so the runtime imports a
  patched `video_vae.py` and the vendored tree is left clean for the next pin move. General,
  not SHA-specific: any modified tracked file that differs across a pin move is now cleared.
- **No model re-download, no weight change, no pin change.** The vendored pin is still
  `871694d` / `0.14.19+ltx25.1`.

#### v3.8.0 — Storyboard, and the install that had been failing for everyone new
- **Storyboard tab** — a concept sentence is planned into a shot list by
  `storyboard_planner.py` (running on the `gemma-3-12b-it-4bit` weights the panel already
  downloads for `/prompt/enhance`, so the feature costs no new bytes), reviewed and edited by
  the user, then rendered through the ordinary queue. Shots are shot GROUPED BY PIPELINE, not
  in story order, because the warm helper holds one pipeline at a time. Plans are durable
  under `state/storyboards/<id>/`; `/storyboard/export` writes a `storyboard.md` + `.json`.
- **The face law** — the planner is structurally prevented from writing silhouettes, backs of
  heads and out-of-frame faces unless the brief asked for them in so many words. `face` is a
  closed enum and Python, not the model, writes the sentence. Person-silhouettes across a
  56-shot corpus: 5 → 0.
- **Install + Update fixed (no issue number — it hit everyone)** — hatchling 1.32.0 made
  ...
  exactly that. Every fresh install and every Update click had been dying at
  `metadata-generation-failed` on every pinned tag since that release landed. Both paths now
  pin the build backend through `pip-build-constraints.txt` (`hatchling<1.32`), and both use
  uv to do it — the `PIP_CONSTRAINT` environment variable no longer works, because pip now
  spawns its build-dependency install with `_PIP_IN_BUILD_IGNORE_CONSTRAINTS=1`.
- **#52, the dead update pill (@Morac2)** — the dirty-tree probe used `git status --porcelain`,
  which lists untracked paths, and installing the H3 pack *creates* untracked paths. The
  update check was suppressed forever. Both probes now pass `-uno`; `minimax-h3-mlx/`,
  `cache/` and `logs/` are root-anchored in `.gitignore`.
- **#46, audio start offset (@blackest)** — both A2V pipelines had accepted `audio_start_time`
  all along; the panel never put a value on the job. Wired end to end (field → `make_job`
  allowlist → `a2v_params`), clamped through a new `_safe_float`, and stored in the sidecar so
  Load Params restores it. The duration slider now warns past 241 frames instead of staying
  silent — a warning, deliberately not a hard cap.
- **#53, stretched first keyframe (@Morac2)** — upstream hard-stretches keyframe 0 onto the
  canvas because it treats it as the geometry anchor, and the panel only ever sends keyframe 0,
  so no setting could avoid it. The reference is now cover-cropped to the exact canvas first,
  which makes upstream's own early return fire and skip the resize entirely. Best-effort: any
  failure falls back to today's behaviour rather than killing the render.
- **H3 menu opens at 46 GB** — v3.7.0 shipped the ~27 GiB Q8 H3 lane and opened the engine to
  48 GB Macs, but `pinokio.js` still gated the menu entry at 60 GB, so the users it was built
  for could not see it.
- **Model-version registry + the conditioning seam** — one declarative table describes a
  generation's packs, cap tiers, text encoder and HQ weight filenames, and every consumer reads
  it instead of a module-level constant. This is what turns "which LTX generation" into a
  curation decision rather than a rewrite.
- **Analytics truth pass** — `docs/ANALYTICS.md` and the README had been claiming "no
  telemetry" and "a fresh clone sends nothing" after the build started shipping a working
  PostHog key. Eleven sites corrected, and the guard suite that should have caught it was red
  and is now green and mutation-tested. Also: clearing the key in Settings is **not** an off
  switch, and every surface now says so — the toggle and `PHOSPHENE_ANALYTICS_DISABLED=1` are
  the only two.
- **Every event now tells PostHog not to locate it** — `$geoip_disable`, a truthy `$ip`
  placeholder and `$process_person_profile: False`, spread last so no call site can override
  them. Nothing was removed, because no location code ever existed here; what changed is that
  the receiver's default derivation is now suppressed instead of merely unmentioned.
- **Vendored MLX port re-pinned** to the fork build `871694d` (`0.14.19+ltx25.1`). Every LTX-2.5
  flag defaults to its 2.3 value, so 2.3 renders exactly what it rendered before.
