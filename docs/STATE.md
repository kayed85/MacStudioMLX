...
> - **The voice gets its own strength — and the shipped default is the graded `1.0`.** The face file's audio-branch deltas are noise and at equal strength they are LOUDER than the voice file's signal (median ‖D‖ 1.45 vs 1.10), so running the voice hotter is what makes it win: parity is 1.2, 1.4 is the first rung with real headroom. The SPLIT CONTROL ships and the help-dot explains the mechanism, but the **default stays at 1.0/1.0 — the pair every graded clip was rendered at and passed on.** 1.4 was the pre-release default and no ear had passed it; a default nobody has listened to is not a default. Raise it when one says so. `character_voice_strength` is asserted into `params` in the same commit that adds it — the allowlist trap, closed with a test rather than a hope.
